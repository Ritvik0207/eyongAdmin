...
export default function UpdateProduct() {
  ...
  const onSubmit = async (data) => {
    try {
      ...
      formData.append("shop", data.shop._id);
      if (data.subCategory?._id) {
        formData.append("subcategory", data.subCategory._id);
      }

      // Add base image if changed
      if (imageFiles.baseImage) {
        formData.append("baseImage", imageFiles.baseImage);
      }

      // Process variants - ensure we keep the _id for existing variants
      const processedVariants = data.variants.map((variant, index) => {
        // Create a copy of the variant without the images array
        const { images, ...variantData } = variant;

        // Keep the _id if it exists (existing variant)
        if (variant._id) {
          variantData._id = variant._id;
        } else {
          // Generate ID for new variants
          variantData.id = `new_${index}`;
        }
      ...
      data.variants.forEach((variant, index) => {
        const variantId = variant._id || `new_${index}`;
        const variantImageFiles = imageFiles[`variants.${index}.images`] || [];

        variantImageFiles.forEach((file, fileIndex) => {
  ...
  const removeVariant = (index) => {
    const variants = form.getValues("variants");
    // Keep track of deleted variant IDs to handle image cleanup
    const deletedVariant = variants[index];
    if (deletedVariant?._id) {
      // Remove any preview images for this variant
      const variantImageKeys = Object.keys(previewImages).filter((key) =>
        key.startsWith(`variants.${index}.images`)
      );
      setPreviewImages((prev) => {
        const newPreviews = { ...prev };
        variantImageKeys.forEach((key) => delete newPreviews[key]);
        return newPreviews;
      });

      // Remove any staged files for this variant
      setImageFiles((prev) => {
        const newFiles = { ...prev };
        variantImageKeys.forEach((key) => delete newFiles[key]);
        return newFiles;
      });
    }
  ...
  const handleImageAttach = (file, path) => {
    if (path.includes("variants")) {
      ...
      // Handle single images (like baseImage)
      setImageFiles((prev) => ({
        ...prev,
        [path]: file,
      }));
  ...
  const ImageDropzone = ({ onDrop, value, path }) => {
    ...
    return (
      <div
        {...getRootProps()}
        className="relative border-2 border-dashed rounded-lg p-4 text-center cursor-pointer transition-colors group"
      >
        <input {...getInputProps()} />
        {displayUrl ? (
